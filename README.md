...
`apiProfile` 可选 `generic` 或 `deepseek`，默认 `generic`。它只选择 Provider 专用的 Responses 能力默认值，不参与模型路由。旧配置仅在字段缺失、Provider ID 为 `deepseek` 且 `baseUrl` 与 `https://api.deepseek.com` 同源时推断为 `deepseek`，下一次保存会固化该值。第三方 Provider 即使使用 `deepseek-v4-flash` 或 `deepseek-v4-pro` 模型名也不会继承 DeepSeek 工具默认值。

Responses Provider 的模型可以通过 `supportsHostedWebSearch` 和 `supportsCustomApplyPatch` 分别向 Codex 暴露服务端 `web_search` 和 Responses `custom` `apply_patch` 工具。DeepSeek Profile 下的 `deepseek-v4-flash` / `deepseek-v4-pro` 默认启用两项能力；其他 Profile 默认关闭。显式 `false` 会持久化并覆盖默认值，`chat-completions` Provider 不允许启用这两项能力。切换协议为 Chat Completions 时，Web UI 会自动清除它们。修改后需要重新运行 `./gatewayctl codex` 并重启 Codex，使新的模型目录生效。
